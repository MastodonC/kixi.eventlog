...
(defn build* [{:keys [vpc-cidr-block subnet-cidr-block security-group key-name instance-type min-count max-count tag load-balancer-name]}]
  (let [tags                [{:key "Name" :value tag}]
        ...
        reservation         (ec2/run-instances {:image-id       "ami-6e7bd919"
        ...
        instances         (get-in reservation [:reservation :instances])]
    ...
    (let [elb (elb/create-load-balancer {:load-balancer-name load-balancer-name
                                         :subnets [subnet-id]
                                         :listeners [{:instance-port 80
                                                      :load-balancer-port 80
                                                      :instance-protocol "HTTP"
                                                      :protocol "HTTP"}]})]
      (doseq [{:keys [instance-id]} instances]
